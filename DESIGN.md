...
components:
  button-primary:
    backgroundColor: zen-sage
    textColor: zen-on-primary
    rounded: pill
    padding: 12px 24px
  button-ghost:
...
**Key Characteristics:**
- Warm oat background with 2% noise grain overlay
...
- **Body** (#5C5A52 light / #B8B6AE dark): Secondary running text when softer than sage is needed.
- **Muted** (#8F9078 light / #6E6C65 dark): Micro-labels, section captions, filter hints.
- **Muted Soft** (#A8A498 light / #8A8880 dark): Placeholders, footnotes, disabled-adjacent copy.

### Hairlines
- **Hairline** (#e8e6e0): Default 1px borders on list cards, inputs, toolbars.
- **Hairline Soft** (#efeee8): Lighter dividers inside panels.
- **Hairline Strong** (#cfcdc4): Hover and expanded list-card borders.

### Semantic
- **Success** (#7A9470 light / #A8B896 dark): Confirmations and positive feedback. Calm, not alarm green.
...
- **FAB:** 64px circle, sage fill, 4px surface border, -top-8 protrusion from nav bar, plus icon rotates 45 degrees when input open.
...
- **Search input:** Small rounded-full, expands width on focus (w-32 to w-48).
...
- **StatusRing:** 280px SVG donut (220px compact), 24px stroke, terracotta to sage gradient arc, center Safe to spend label. Rendered on Pulse hero in compact mode.
...
### Dont:
- **Dont** use #000, #fff, navy-and-gold fintech palettes, or generic SaaS hero-metric layouts.
- **Dont** add colored side-stripe borders on cards, alerts, or list items.
- **Dont** use gradient text (background-clip: text) for amounts or headings.
- **Dont** use glassmorphism (backdrop-blur + transparency) as the default for every surface; reserve for shell and input areas.
- **Dont** import Lucide or other external icon libraries; extend icons.json instead.
- **Dont** nest cards inside cards; use spacing and subtle borders to separate groups.
- **Dont** use bounce or elastic easing on UI transitions.
- **Dont** show punitive red, OVERDUE, or shame language for missed entries.
- **Dont** open modals when inline expansion or bottom sheets suffice.
- **Dont** introduce off-palette colors (raw Tailwind blue/emerald on dashboard cards) without mapping to Zen semantic tokens.